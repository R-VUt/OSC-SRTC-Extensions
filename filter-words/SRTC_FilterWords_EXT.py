...
def extension_execute():
    with open("to_filtering.txt", "r", encoding="utf-8") as f:
        to_filtering = [x.lower() for x in f.read().splitlines() if x]

    message = flask.request.args.get("message")
    if not message:
        return "Message is not exist", 400

    for word in to_filtering:
        if word in message.lower():
            log(f"[INFO] Filtered: {message}")
...
def main_window():
    ...
    title_label = tkinter.Label(root, text="OSC-SRTC Word Filter Extension V1", font="Helvetica 12 bold")
    title_label.pack()
